...
const DeleteResumeConfirmationModal = ({
  toggleModal,
  resumeID,
  setReset,
}: upgradeProps) => {
  ...
  const handleDeleteResumeByID = async () => {
    setIsloading(true);
    try {
      const method = 'DELETE';
      //   console.log('data', data);

      const route = ENUM.DELETE_SINGLE_RESUME + '/' + resumeID;
        ...
        <div className="relative bg-white rounded-lg shadow ">
          ...
          <div className="p-4 md:p-5 text-center">
            <h2 className="text-2xl text-[#000] font-semibold mb-[22px] w-[304px] mx-auto">
              You are about to delete a Resume
            </h2>
            <h3 className="mb-5 text-base font-normal text-[#EB5757]">
              NOTE: Once your resume is Deleted, you can no longer have access
              to it again.
            </h3>
            <div className="flex items-center gap-[12px]">
              <ButtonRounded
                onClick={handleDeleteResumeByID}
                isLoading={isLoading}
                type="button"
...
export default DeleteResumeConfirmationModal;
